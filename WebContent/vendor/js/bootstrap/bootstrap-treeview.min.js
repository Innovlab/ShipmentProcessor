...
;(function($, window, document, undefined) {
	...
	var Tree = function(element, options) {
		...
		this.selectedNode;
		this.lvls={
			level1:{text:"projectName",nodes:"testSuiteList"},
			/*level2:{text:"testSuiteName",nodes:null},*/
			/*level2:{text:"testSuiteName",nodes:"testCaseList"},
			level3:{text:"testCaseName",nodes:null}*/
		};
		this._init(options);
	};
	...
	Tree.prototype = {
		...
		_init: function(options) {
			...
			this._render();
			//this._setDefaultSelectedNode()
		},

		_unsubscribeEvents: function() {

			this.$element.off('click');
			this.$element.off('setToDefault');
		...
		_subscribeEvents: function() {
			...
			//this.$element.on('setToDefault', $.proxy(this._setDefaultSelectedNode, this));
		},
		...
		_clickHandler: function(event) {
			...
			var target = $(event.target),
				...
				/*else */
				if (node) {
					if (this._isSelectable(node) && node.nodeId!=0) {
						this._toggleNodes(node);
		...
		// to set default to projects node
		...
		_setInitialLevels: function(nodes, level) {
			if (!nodes) { return; }
			level += 1;
			var ln=this.lvls["level"+level+""].nodes;
			var lt=this.lvls["level"+level+""].text;
			var self = this;
		...
		_buildTree: function(nodes, level) {
			...
			var self = this;
			$.each(nodes, function addNodes(id, node) {
				if(node.globalTestSuite){
					self.autoRecord=node.globalTestSuite;
				}
				node.nodes=node[ln];
				node.text=node[lt];
				...
				if(level==1 && node.projectId){
					treeItem.append($(self._template.projectNode));
				}/*else if (node.nodes) {
				...
				if (self.options.enableLinks) {
					...
					// otherwise just text
					/*treeItem.append($(self._template.text)
							.append(node.text)
					);*/
				...
				/*if(level==1 && node.projectId){
					treeItem.append($(self._template.refresh));
				}*/

				// Add tags as badges
				if(level===2 && node.testSuiteId==self.autoRecord){
					node.tags=["AR"]
				}
		...
		_template: {
			...
			item: '<li class="list-group-item" style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"></li>',
			indent: '<span class="indent"></span>',
			expandCollapseIcon: '<span class="expand-collapse" data-toggle="tooltip" data-placement="left" title="expand/collapse"></span>',
			icon: '<span class="icon"></span>',
			link: '<a href="#" style="color:inherit;"></a>',
			badge: '<span class="badge" data-toggle="tooltip" data-placement="left" title="Auto Record in progress"></span>',
			projectNode:'<span class="glyphicon glyphicon-tasks" style="font-size:15px;"></span>'
			//text:'<span style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"></span>'
			//refresh:'<span style="float:right;margin-right: 3px;" data-toggle="tooltip" data-placement="left" title="refresh testcase(s)"  class="glyphicon glyphicon-refresh"></span>'
			
		},

		_css: '.treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:6px;margin-right:6px}.treeview span.expand-collapse{width:1rem;height:1rem}.treeview span.icon{margin-left:6px;margin-right:5px}'
